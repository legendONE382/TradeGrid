...
app.post('/api/listings', (req, res) => {
  ...
  values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(seller_id, title, category, quantity, unit, price, town || null, region || null, delivery_available ? 1 : 0, pickup_available === false ? 0 : 1);
  res.status(201).json({ id: r.lastInsertRowid });
});
...
app.patch('/api/deliveries/:id/accept', (req, res) => {
  db.prepare("update deliveries set transporter_id=?, status='accepted' where id=? and status='open'").run(req.body.transporter_id, req.params.id);
  res.json({ ok: true });
});
...
app.post('/api/messages', (req, res) => {
  const { from_user_id, to_user_id, order_id, body } = req.body;
  const r = db.prepare('insert into messages (from_user_id, to_user_id, order_id, body) values (?, ?, ?, ?)').run(from_user_id, to_user_id, order_id || null, body);
  res.status(201).json({ id: r.lastInsertRowid });
});
